...
bool TcpHub::ping(QString server, int port, QString uuid)
{
    ...
    if (!waitSignal(&socket, SIGNAL(connected()), 1000)) {
        return false;
    }
    ...
    auto res = waitForLine(&socket, 1000);
...
void TcpHub::newTcpHubConnection()
{
    ...
    QString connStr = waitForLine(socket, 5000);
...
bool TcpHub::waitSignal(QObject *sender, QString signal, int timeoutMs)
{
    QEventLoop loop;
    QTimer timeoutTimer;
...
QString TcpHub::waitForLine(QTcpSocket *socket, int timeoutMs)
{
    QEventLoop loop;
    QTimer timeoutTimer;
    timeoutTimer.setSingleShot(true);
